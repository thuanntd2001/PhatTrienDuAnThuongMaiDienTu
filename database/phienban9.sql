...
-- Thu 16 Feb 2023 07:50:55 AM +07
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`PHIEUNHAP` (
  ...
  `NHANVIEN_MANV` BIGINT NOT NULL,
  PRIMARY KEY (`MAPN`),
  INDEX `fk_PHIEUNHAP_NHANVIEN1_idx` (`NHANVIEN_MANV` ASC)  ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`SANPHAM` (
  ...
  PRIMARY KEY (`ID`),
  INDEX `FK_THUCDON_LOAITHUCUONG` (`LOAI` ASC)  ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`CTPN` (
  ...
  `ID` BIGINT NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`ID`),
  INDEX `fk_CTPN_PHIEUNHAP1_idx` (`MAPN` ASC)  ,
  INDEX `fk_CTPN_SANPHAM1_idx` (`SANPHAM` ASC)  ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`DONDATHANG` (
  ...
  PRIMARY KEY (`MADDH`),
  INDEX `FK_HOADON_NHANVIEN` (`NVXACNHAN` ASC)  ,
  INDEX `fk_DONDATHANG_KHACHHANG1_idx` (`MAKH` ASC)  ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`GIOHANG` (
  ...
  `ID` BIGINT NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`ID`),
  INDEX `fk_CHITIETDDH_copy1_KHACHHANG1_idx` (`MAKH` ASC)  ,
  INDEX `fk_CHITIETDDH_copy1_SANPHAM1_idx` (`SANPHAM` ASC)  ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`NHAN_SANPHAM` (
  ...
  `TENNHAN` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`ID`),
  INDEX `fk_NHAN_has_SANPHAM_SANPHAM1_idx` (`SANPHAM_ID` ASC),
  INDEX `fk_NHAN_SANPHAM_NHAN1_idx` (`TENNHAN` ASC),
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`TUKHOA_NHAN` (
  ...
  `TENNHAN` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`ID`),
  INDEX `fk_TUKHOA_NHAN_TUKHOA1_idx` (`TENTUKHOA` ASC) ,
  INDEX `fk_TUKHOA_NHAN_NHAN1_idx` (`TENNHAN` ASC) ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`USER` (
  ...
  INDEX `FK_USERTB_CHUCVU` (`ROLEID` ASC) ,
  INDEX `FK_USERTB_NHANVIEN` (`MANV` ASC) ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`USERKH` (
  ...
  PRIMARY KEY (`USERNAME`),
  INDEX `FK_USERTB_CHUCVU` (`ROLEID` ASC) ,
  INDEX `fk_USERKH_KHACHHANG1_idx` (`MAKH` ASC) ,
...
CREATE TABLE IF NOT EXISTS `KINHDOANHIOT`.`CTDDH` (
  ...
  `ID` BIGINT NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`ID`),
  INDEX `fk_CTPX_SANPHAM1_idx` (`SANPHAM` ASC) ,
  INDEX `fk_CTDDH_DONDATHANG1_idx` (`MADDH` ASC) ,
